...
//        swPin16UARTTX.setEnabled(false);
    ...
    private boolean getSPDIFOUTStatus() {
        String status = execSuCmd("getbootenv ubootenv.var.khadas_multiple_spdifout_id");
            Log.d(TAG, "getSPDIFOUTStatus " + status);
    ...
    private void setSPDIFOUTStatus(boolean status) {
        ...
        try {
            if (status) {
                 Runtime.getRuntime().exec("setbootenv ubootenv.var.khadas_multiple_spdifout_id 1");
             } else {
                 Runtime.getRuntime().exec("setbootenv ubootenv.var.khadas_multiple_spdifout_id 0");
             }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private boolean getUartCStatus() {
        String status = execSuCmd("getbootenv ubootenv.var.khadas_multiple_uartc_id");
    ...
    private void setUartCStatus(boolean status) {
        ...
        if(getUartCStatus() == status) {
         ...
         try {
             if (status) {
                 Runtime.getRuntime().exec("setbootenv ubootenv.var.khadas_multiple_uartc_id 1");
             } else {
                 Runtime.getRuntime().exec("setbootenv ubootenv.var.khadas_multiple_uartc_id 0");
             }
          } catch (IOException e) {
                  e.printStackTrace();
          }
    ...
    private boolean getI2SStatus() {
        String status = execSuCmd("getbootenv ubootenv.var.khadas_multiple_i2s_id");
        Log.d(TAG, "getI2SStatus " + status);
        if (!TextUtils.isEmpty(status) && status.equals("0")) {
            return false;
        }
        return true;
    ...
    private void setI2SStatus(boolean status) {
        ...
        try {
            if (status) {
                 Runtime.getRuntime().exec("setbootenv ubootenv.var.khadas_multiple_i2s_id 1");
             } else {
                 Runtime.getRuntime().exec("setbootenv ubootenv.var.khadas_multiple_i2s_id 0");
             }  
        } catch (IOException e) {
            e.printStackTrace();
        }
