...
public class AutoPairActivity extends Activity implements BluetoothDevicePairer.EventListener {
    ...
    private static final String BLE_REMOTE_NAME = "persist.bleremote.name";

    private static class MessageHandler extends Handler {

        private WeakReference<AutoPairActivity> mActivityRef = new WeakReference<>(null);
        private WeakReference<Context> mContextRef = new WeakReference<>(null);

        public void setActivity(AutoPairActivity activity) {
        ...
        public void handleMessage(Message msg) {
            final AutoPairActivity activity = mActivityRef.get();
            ...
            switch (msg.what) {
                ...
                case MSG_PAIRING_COMPLETE:
                    ...
                    activity.showToast(context,context.getString(R.string.pair_connected),Toast.LENGTH_SHORT);
                ...
                case MSG_OP_TIMEOUT:
                    ...
                    activity.showToast(context,context.getString(R.string.autopait_timeout_error),Toast.LENGTH_LONG);
                ...
                case MSG_TRIGGER_SELECT_DOWN:
                    if (DEBUG) {
                        Log.d(TAG, "handleMessage MSG_TRIGGER_SELECT_DOWN");
                    }
                ...
                case MSG_TRIGGER_SELECT_UP:
                    if (DEBUG) {
                        Log.d(TAG, "handleMessage MSG_TRIGGER_SELECT_UP");
                    }
                    activity.sendKeyEvent(KeyEvent.KEYCODE_DPAD_CENTER, false);
                    break;
                case MSG_START_AUTOPAIR_COUNTDOWN:
                    if (DEBUG) {
                        Log.d(TAG, "handleMessage MSG_START_AUTOPAIR_COUNTDOWN");
                    }
                ...
                case MSG_AUTOPAIR_TICK:
                    if (DEBUG) {
                        Log.d(TAG, "handleMessage MSG_AUTOPAIR_TICK");
                    }
    ...
    private void showToast(Context context, String str, int time){
        if(mToast != null ){
            mToast.cancel();
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_autopair);
        if (DEBUG) {
            Log.d(TAG, "onCreate");
        }
            ...
            switch (status) {
                ...
                case BluetoothDevicePairer.STATUS_PAIRING:
                    ...
                    if(mPairDeviceName!=null){
                        showToast(AutoPairActivity.this,AutoPairActivity.this.getString(R.string.pair_pairing),Toast.LENGTH_SHORT);
                    }
                    break;
                case BluetoothDevicePairer.STATUS_CONNECTING:
                    if(mPairDeviceName!=null){
                        showToast(AutoPairActivity.this,AutoPairActivity.this.getString(R.string.pair_connecting),Toast.LENGTH_LONG);
                    }
                    state = "BluetoothDevicePairer.STATUS_CONNECTING";
                    break;
                case BluetoothDevicePairer.STATUS_ERROR:
                    if(mPairDeviceName!=null){
                        showToast(AutoPairActivity.this,AutoPairActivity.this.getString(R.string.pair_status_error),Toast.LENGTH_SHORT);
                    }
    ...
    private void stopScanning() {
        mBluetoothDevices.clear();
        mBluetoothPairer.cancelScanning();
    }
